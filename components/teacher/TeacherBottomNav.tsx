...
import { ClassroomIcon, QuizBankIcon, ReportsIcon, ProfileIcon, HomeIcon } from '../icons';
import { useTranslations } from '../../hooks/useTranslations';

export type TeacherDashboardView = 'home' | 'classroom' | 'quizBank' | 'reports' | 'profile' | 'announcements';

interface TeacherBottomNavProps {
    activeView: TeacherDashboardView;
    onNavigate: (view: TeacherDashboardView) => void;
}

const TeacherBottomNav: React.FC<TeacherBottomNavProps> = ({ activeView, onNavigate }) => {
    const { t } = useTranslations();

    const navItems = [
        { view: 'classroom', label: t('classroom'), icon: ClassroomIcon },
        { view: 'quizBank', label: t('quizBank'), icon: QuizBankIcon },
        { view: 'home', label: t('home'), icon: HomeIcon },
        { view: 'reports', label: t('reports'), icon: ReportsIcon },
            ...
            <nav className="relative h-24">
                ...
                <div
                    ...
                    style={{
                        ...
                        opacity: activeIndex !== -1 ? 1 : 0, // Hide if no item is active (e.g., announcements view)
                    }}
                ></div>
